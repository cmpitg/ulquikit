...
boolean (e.g. in case of ``1``, ``t``, ``true``, ``false``, ...).  See the
test section of the implementation below for detailed information.
...
AsciiDoc files [TODO: CHECK] recursively (using <<func/list-all-adocs,`list-all-adocs`>>)
...
(defun generate-src (&key (from "src") (to "generated-html"))
