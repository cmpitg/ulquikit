...
Ulquikit commands are Common Lisp functions residing in `commands/`.  By
convention, each command has its own Common Lisp source code whose name is the
same as the command, i.e. the `generate-src` command has its file
`commands/generate-src.lisp`.
