...
`include-snippet!` takes 2 mandatory arguments and an optional hash table
representing:
* the current list of snippets,
* the targeted snippet -- the one is being processed, this snippet should be a
  part of the current list of snippets,
* and the hash table determining whether or not a snippet has been processed
  (to prevent circular dependency)

This function returns multiple values, in the following order:
* the list of snippets that have been processed,
* the targeted snippet that has been included,
* and the hash table determining whether or not a snippet has been processed
  after the targeted snippet has been included

`include-snippet!` works by browsing ``target``'s content, one line at a time,
then replacing lines with `include::` directives with the corresponding __code
snippet__s in `snippets`.  If no snippet is found, the line is left as-is.
`snippets` should contain `target`.
...
* Also because of the above reason, `snippets` should be able to receive the
  changes across all recursive calls to `include-snippet`.

* If snippet A includes snippet B, then snippet B includes snippet A again, we
  have a circular dependency problem.  To prevent this, a list of currently
  included snippet must be kept track of.  If a snippet has already been in
  the track, include it but don't process it.

WARNING: The result of circular dependency is *undefined*.  Make sure your
snippets are well managed.
...
(defun include-snippet! (snippets target
                         &optional
                           (processed? (make-hash-table :test #'equalp)))
  "Processed `target' by replacing all of its \"include\" directives with the
corresponding snippets found in `snippets'.  `processed?' helps solving
circular dependency by recording the list of processed snippets up to the
current point.

This function returns multiple values:
* the processed `snippets' after `target' is included,
* `target' after included,
* `processed?' after `target' is included"
  (declare (snippets   snippets)
           (snippet    target)
           (hash-table processed?))
  (let* ((snp/type (snippet-type target))
         (snp/name (snippet-name target))
         (snp/repr (cons snp/type snp/name))
         (lines    (snippet-lines target)))
    ;; Only process `target' if not included and this snippet is in the list
    (unless (or (gethash snp/repr processed?)
                (and (not (gethash snp/name (snippets-file snippets)))
                     (not (gethash snp/name (snippets-code snippets)))))
...
(define-test test-include-snippet
  (let* ((snp/file (make-snippet :name "/tmp/tmp.lisp"
         ...
         (snippets (let* ((res (make-snippets))
                     ...
                     res)))
    (multiple-value-bind (snippets _ processed?)
        (include-snippet! snippets snp/code/A)
      (assert-true (gethash '(:code . "A") processed?) "A is now processed")
      (assert-equal '("World"
                      "Hello"
                      ";; include::D")
                    (snippet-lines snp/code/A)))))
...
(defun snippet-exists? (snippet snippets)
  "Determines if `snippet' is in `snippets'."
  (declare (snippet snippet)
           (snippets snippets))
