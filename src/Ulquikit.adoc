...
`include-snippet` should take 2 arguments representing 2 ``snippets``s:
`(include-snippet snippets target)`.  `include-snippet` works by browsing
``target``'s content one line at a time, then replacing lines with `include::`
directives with the corresponding __code snippet__s in `snippets`.  If no
snippet is found, the line is left as-is.
