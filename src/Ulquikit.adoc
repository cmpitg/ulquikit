...
automatically detected.  A command is actually a Common Lisp package, residing
in `commands/` directory.  Each has `help` and `run` functions, which are
called when user runs or gets help, respectively.  Should one of the function
does not exist, the correponding functionality does nothing.
...
version.  E.g. if command `extract-text` is defined in `extract-text.lisp` and
command `help` is defined in `help.lisp`, `extract-text` is loaded before
`help` as `extract-text.lisp` precedes `help.lisp` alphabetically.  If
`extract-text` is onec again defined/modified in `z-extract-text.lisp`, those
changes override `extract-text.lisp` as `z-extract-text.lisp` comes after
`extract-text.lisp` in alphabetical order.


As a rule of thumb, each command provides at least 2 functions: one to run,
and the other to get help:
