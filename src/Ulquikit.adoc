...
`include-file-snippets` function.  This function takes list of snippets (of an
instance of `snippets` struct) returns an instance of `snippets` with all file
...
file snippets included.  Note that this function modifies its argument."
  (declare (snippets snippets))
...
(defun write-src-files (snippets to)
  ...
  (iter (for (name snippet) in-hashtable (snippets-file snippet))
        (let ((path (uiop:merge-pathnames*
                     name
                     (uiop:merge-pathnames*
                      to
                      (uiop:getcwd))))
              (content (join-lines (snippet-lines snippet))))
