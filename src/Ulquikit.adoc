...
(defpackage #:ulquikit-tests
  (:use :cl :ulquikit :alexandria :cl-ppcre :lisp-unit :iterate))
----
...
(deftest test-parse-snippet-title ()
  ...
  (dolist (el '((".file::"    . (:file ""))
    ...
    (let ((title    (first el))
          (expected (rest  el)))
      (multiple-value-bind (type name) (parse-snippet-title title)
        (is (equal expected (list type name)))))))
...
(deftest test-get-all-adocs ()
  ...
  (let* ((files '("a.adoc"
         ...
         (temppath (merge-pathnames "ulquikit/test-get-all-adocs/"
                                    (make-pathname
                                     :directory `(:absolute ,(uiop:getenv "TMPDIR")))))
         (expected (iterate
                     (for path in files)
                     (when (scan "\\.adoc$" path)
                       (collect (merge-pathnames temppath path))))))
    ;; Setup
