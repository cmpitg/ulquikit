...
When user invokes a command, the corresponding function gets called.  This is
...
To make Ulquikit as user-friendly as possible, when a user executes as invalid
...
We also have `run-help` to display help of a command.  `run-help` simply prints to
...
To conclude this section, let's fill in the the last missing piece -- the
command counterpart of the <<func/generate-html,`generate-html`>> function:
...
== The `ulqui` executable

Up to this point, Ulquikit is perfectly usable as a library.  With
https://www.quicklisp.org/beta/[Quicklisp], it could be loaded very
straightforwardly:

[source,lisp]
----
;; Symlinking Ulquikit to Quicklisp local projects directory
;; ln -s /path/to/ulquikit/asdf ~/quicklisp/local-projects/

;; Loading
(ql:quickload "ulquikit")

;; Using
(ulquikit:generate-src :from "/path/to/literate/source" :to "/path/to/generated/source")
(ulquikit:generate-html :from "/path/to/literate/source" :to "/path/to/generated/html/doc")

----

The next step is to turn Ulquikit into a executable named `ulqui` so it could
be used as a standalone program.

=== General help

Invoking `ulqui`, `ulqui --help`, or `ulqui help` should bring up the general
usage of Ulquikit.  This behavior is defined with the `help` command.  This
command should detect all current Ulquikit commands (defined by the
<<defcmd,`defcmd`>> macro) and output their short summary.

.file::commands/help.help.txt
[source,text]
----
Usage: ulqui <command> [options] ...
...
Use 'ulqui version' or 'ulqui --version' to display current version of Ulquikit.

----

The `help` command could then be implemented as followed.  Note that it
replaces documentation of `help` function and shows the actual description of
`help` command itself, not Ulquikit.

.file::commands/help.lisp
[source,lisp,linenums]
----
;; include::license-header
...
Putting all things together, we have the following entry point for using
Ulquikit as a standalone program: the `main` function.  By default, running
`ulqui` usually means users need some help.  Thus running `ulqui` is
equivalent to running `ulqui help`.

.code::ulquikit-cmd/main
[source,lisp,linenums]
----
(in-package #:ulquikit-cmd)
...
.file::commands/version.help.txt
[source,text]
----
Usage: ulqui version

Display the current running version of Ulquikit.

----

.file::commands/version.lisp
[source,lisp,linenums]
----
;; include::license-header
...
As Ulquikit grows, the need for a tool to manage its source code and release
arises.
http://en.wikipedia.org/wiki/List_of_Hollows_in_Bleach#Ulquiorra_Schiffer[Schiffer]
is born to fulfill the need.  Schiffer is written in Common Lisp, build with
http://www.xach.com/lisp/buildapp/[Buildapp] and used as a standalone
executable in the project root.  It could be thought as a Ulquikit-specific
Make-like/Rake-like tool.

=== Generating source code and HTML documentation

.code::schiffer/generate
[source,lisp,linenums]
----
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
Each time Schiffer it built, its development version at
`<project-root>/build/schiffer` is created.  When the development version is
stable, it could replace the current version at `<project-root>/schiffer`.
...
----
;; (defun mark-stable ()
;;   (format t "⇨ Removing current stable")
;;   (uiop:delete-directory-tree (ulqui-dir-to "release")
;;                               :if-does-not-exist :ignore)

;;   (format t "⇨ Creating stable directory: release")
;;   (uiop:ensure-all-directories-exist (list (ulqui-dir-to "release")))

;;   (format t "⇨ Copying build to release")
;;   (uiop:copy-file (ulqui-dir-to "build/ulqui")
;;                   (ulqui-dir-to "release/ulqui"))

;;   (format t "⇨ Copying docs")
;;   (format t "   generated-html -> release/ulquikit/docs")
