...
User can then choose to generate documentation or source code with procesesd
snippets.

In a high-level perspective, the basic functionality of a literate programming
tool like Ulquikit is to support source code and documentation generation.
Hence, the following functions are the most important:

* <<func/extract-snippets-from-file,`extract-snippets-from-file`>>

* <<func/include-snippet,`include-snippet!`>>, serving as a building block to
