...
* Ruby 2.3.3p222 with https://rvm.io[RVM] (Ruby Version Manager)
...
task "gen-src" do
  sh "ulqui generate-src --from src/ --to generated-src/ --recursive false"
end

task "gen-docs" do
  sh "ulqui generate-html --from src/ --to docs/ --recursive false"
end

task "gen-all" => ["gen-src", "gen-docs"]
