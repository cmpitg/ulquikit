...
(define +ulqui-script-path+
  (simplify-path (or (syntax-source #'here)
...
* If a file is run as script, +syntax-source+ will return +#f+, and
  +find-system-path+ returns path to the scirpt, which is what we want
* If a file is run with +racket <filename>+, +syntax-source+ returns the
