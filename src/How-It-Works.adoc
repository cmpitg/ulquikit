...
IMPORTANT: Ulquikit follows
http://en.wikipedia.org/wiki/Convention_over_configuration[convention over
...
=== Generating source code and HTML documents, all at once
...
(define (mark-stable)
  ...
  (displayln "=> Creating stable directory: release/ulquikit")
  (make-directory* "release/ulquikit")
  (newline)

  (displayln "=> Marking current generated source as stable")
...
=== Running Ulquikit current dev version

Running current Ulquikit dev version by calling +generated-src/bin/ulqui+.
...
This comes in handy at times.  This script assumes that *users have already
...
.code::quick-install/asciidoc
[source,sh,linenums]
----
if [ `which asciidoctor >/dev/null 2>&1 && echo true || echo false` == "true" ]; then
...
[source,sh,linenums]
----
if [ `(raco pkg show | grep rackjure) >/dev/null 2>&1 && echo true || echo false` == "true" ]; then
    echo "-> Rackjure found."
else
