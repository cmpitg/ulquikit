    ...
    (if (empty? args)
        ...
        (let* ([current-arg (first args)]
          ...
          (cond [(is-argument? current-arg)
                 (let* ([arg/converted (if-let [value (string->number current-arg)]
                                         value
                                         current-arg)])
                  (if (null? last-keyword)
                      (parse null
                             more
                             (append result (list arg/converted)))
                      (parse null
                             more
                             (append result (list (list last-keyword arg/converted))))))]

                [(is-option? current-arg)
                 (if (null? last-keyword)
                     (parse (parse-keyword current-arg)
                            more
                            result)
                     (parse (parse-keyword current-arg)
                            more
